...
| Qwen3.8 FP8 target-head greedy fusion | Fuse exact FP8 vocabulary projection with BF16-rounded greedy argmax and replace full-logit greedy acceptance with token-only acceptance; C1 greedy graphs use the same fixed fused topology while sampled/C2+ graphs retain the full-logit route | Focused FP8 A16 linear, speculative-round, and DFlash2 suites pass. The physical RTX 5090 graph A/B was neutral/slightly slower (`18.6411` vs `18.6593` ms GPU; `18.6549` vs `18.6693` ms wall) with identical `18/70` acceptance, so this remains a correctness/working-set optimization without a standalone throughput claim |
