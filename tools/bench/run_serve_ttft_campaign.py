...
def _server_command(profile: str, weights: Path) -> list[str]:
    return [
        str(SERVE),
        str(weights),
        "--host",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    case_names = CAMPAIGNS[args.campaign]
    ...
    output_dir = (
        ...
        else (OUTPUT_ROOT / f"{_stamp()}-{args.campaign}").resolve()
    )
            ...
            plans.append(
                {
                    ...
                    "server_command": _server_command(definition.profile, runtime_weights),
    ...
    print(
        f"TTFT campaign={args.campaign} runs={total} output={output_dir}",
        flush=True,
    )
    ...
    runs = load_runs(valid_runs) if valid_runs else []
