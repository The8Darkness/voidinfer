...
`profiles/bench/ttft/`, and writes the summary. The case catalog, exact profiles, TTFT boundary, and
fixture qualification are documented in the dedicated README.
...
exposure, and decode Host/Device-wait time per round received from the schema-v16 serving records.
...
pool. With an explicit capacity, size the shared pool for the configured active-request count;
otherwise admission can serialize a nominal C4/C8 wave even though the server itself starts with
that concurrency. The runner sets `--max-pending-requests` to the point concurrency so all wave
requests can be admitted. A point is intentionally not resumable: combining fragments from separate
server processes would not preserve either a steady interval or one continuous makespan.
