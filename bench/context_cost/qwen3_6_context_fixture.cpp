...
#include <limits>
#include <map>
// pi-lens-ignore: clang:typename_nested_not_found
#include <set>
#include <span>
#include <stdexcept>
#include <string>
...
std::uint64_t attention_pairs(std::uint32_t prefix, std::uint32_t suffix) {
    ...
    const auto checked_multiply = [](std::uint64_t lhs, std::uint64_t rhs) {
    ...
    const std::uint64_t prefix_pairs = checked_multiply(prefix, suffix);
    const std::uint64_t triangular = [&] {
        const bool suffix_is_odd     = (suffix & 1U) != 0;
        const std::uint64_t left      = suffix_is_odd ? suffix : suffix / 2U;
        const std::uint64_t right     = suffix_is_odd ? (suffix + 1U) / 2U : suffix + 1U;
        return checked_multiply(left, right);
    }();
    if (prefix_pairs > max_value - triangular) {
        throw std::overflow_error("prefill attention-pair count exceeds uint64");
    }
    return prefix_pairs + triangular;
}
