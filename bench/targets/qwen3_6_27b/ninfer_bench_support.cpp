...
KvCacheStorage parse_kv_cache(std::string_view text) {
    ...
    if (text == "vericache-nvfp4") { return KvCacheStorage::VeriCacheNvfp4; }
    throw std::invalid_argument(
        "--kv-dtype must be bf16, int8, fp8, nvfp4, or vericache-nvfp4");
}
...
std::string usage_text(std::string_view program) {
    ...
    out << "Usage: " << program << " --weights <artifact.ninfer> [options]\n\n"
        ...
        << "  --kv-dtype <bf16|int8|fp8|nvfp4|vericache-nvfp4>  KV cache storage (default: bf16)\n"
...
BenchOptions parse_args(int argc, char** argv) {
    ...
    if (options.kv_cache == KvCacheStorage::VeriCacheNvfp4 && options.mtp_draft_tokens == 0) {
        throw std::invalid_argument(
            "--kv-dtype vericache-nvfp4 requires --mtp-draft-tokens greater than zero");
    }
    return options;
}
