...
bool dflash2_adaptive_k_storage_enabled() noexcept {
...
PersistentLayout persistent_layout(const SequencePlanImpl& plan) {
    ...
    if constexpr (Variant::supports_dflash) {
        if (plan.features.dflash()) {
            ...
            // DFlash2's local KV is part of the speculative drafter, not the authoritative
            // target KV cache. Keep it in BF16 so the proposal path does not inherit the lossy
            // VeriCache L0 representation. Legacy DFlash1 retains its existing quantized local
            // cache behavior.
            const bool dflash_local_quantized = dflash_nvfp4 && !dflash_v2;
            std::uint32_t protected_recent = 0;
            std::uint32_t protected_anchor = 0;
            if (dflash_local_quantized && plan.hierarchical_vericache.enabled &&
            ...
            // DFlash2 keeps its speculative local L0 lossless in BF16. The target decoder cache
            // below remains independently controlled by plan.kv_storage (including OSCAR-Q2),
            // and DFlash1 keeps the legacy NVFP4/OSCAR local contract.
            ...
            state_image_spec.dflash_local = qwen3_6::DFlashLocalStateSpec{
                ...
                .dtype               = dflash_local_quantized ? DType::U8 : DType::BF16,
                .quant_group         = dflash_local_quantized
                                           ? (use_oscar ? 128 : qwen3_6::kKvNvfp4QuantGroup)
                                           : 0,
                .quant_bits          = dflash_local_quantized ? l0_bits : static_cast<std::uint8_t>(0),
                .quantization        = dflash_local_quantized
                                           ? (use_oscar
                                                  ? CyclicKVCacheQuantization::OscarAffine
                                                  : CyclicKVCacheQuantization::Nvfp4)
                                           : CyclicKVCacheQuantization::Auto,
            };
            if (dflash_local_quantized) {
                // Legacy quantized DFlash local state may use an explicitly selected host format.
                // DFlash2 does not enter this branch: its host mirror inherits the BF16 local
                // state, so no compressed shadow or conversion is allocated.
    ...
    if constexpr (Variant::supports_dflash) {
        if (plan.features.dflash()) {
            ...
            if constexpr (DFlashConfig::is_v2) {
                if (dflash2_adaptive_k_storage_enabled()) {
                    dflash.adaptive_replay_records = plan_gdn_replay_records(
                        builder, GdnReplayRecordSpec{
                                     .layers          = TextConfig::gdn_layers(),
                                     .record_capacity = static_cast<std::int32_t>(plan.max_concurrency),
                                     .width           = static_cast<std::int32_t>(plan.draft_window + 1U),
...
WorkspacePlan build_workspace_plan(const SequencePlanImpl& plan) {
    ...
    const auto attention_stage = [&](WorkspaceLayoutBuilder& layout, std::int32_t first,
        ...
        const bool rotated = oscar_internal::rotated_bf16_mode_enabled();
        const bool matched = oscar_internal::matched_fp32_mode_enabled();
        if (rotated || matched) {
            if (rotated) {
                // D1.1 retains these banks for the original rotated-BF16 diagnostic modes. The
                // matched D1.2 path bypasses them and uses only the FP32 banks below.
                matrix(layout, DType::BF16, TextConfig::query_size, last);
                matrix(layout, DType::BF16, TextConfig::kv_size, last);
                matrix(layout, DType::BF16, TextConfig::kv_size, last);
            }
            if (matched || oscar_internal::rotation_precision_mode() !=
                                oscar_internal::RotationPrecision::Bf16Materialized) {
                // Matched control/candidate execution retains FP32 Q/K/V, attention output, and
                // recovered output. This is intentionally a correctness-only diagnostic profile.
                matrix(layout, DType::FP32, TextConfig::query_size, last);
                matrix(layout, DType::FP32, TextConfig::kv_size, last);
                matrix(layout, DType::FP32, TextConfig::kv_size, last);
                matrix(layout, DType::FP32, TextConfig::query_size, last);
                matrix(layout, DType::FP32, TextConfig::query_size, last);
            }
