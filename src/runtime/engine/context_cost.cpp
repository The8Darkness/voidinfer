...
std::uint64_t q32_product_ns(std::uint64_t coefficient, std::uint64_t units) noexcept {
    ...
    // Split the coefficient at the fixed-point boundary so every intermediate product fits in
    // 64 bits. This is ceil(coefficient * units / 2^32), saturated to u64::max.
    constexpr std::uint64_t fraction_mask = (std::uint64_t{1} << 32U) - 1U;
    const std::uint64_t whole              = coefficient >> 32U;
    const std::uint64_t fraction           = coefficient & fraction_mask;
    std::uint64_t result                   = saturating_product(whole, units);
    if (result == std::numeric_limits<std::uint64_t>::max()) {
        return result;
    }

    const std::uint64_t low_product = fraction * (units & fraction_mask);
    const std::uint64_t fractional_whole =
        fraction * (units >> 32U) + (low_product >> 32U);
    result = saturating_add(result, fractional_whole);
    if (result == std::numeric_limits<std::uint64_t>::max()) {
        return result;
    }
    return (low_product & fraction_mask) == 0 ? result : saturating_add(result, 1U);
}
