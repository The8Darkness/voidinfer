...
#pragma unroll
    for (int item = 0; item < 4; ++item) {
        ...
        k_values[item] = cyclic_oscar_decode_value<SourceBits>(source_k + source_row,
                                                                source_k_metadata + source_scale_row,
                                                                d);
        v_values[item] = cyclic_oscar_decode_value<SourceBits>(source_v + source_row,
                                                                source_v_metadata + source_scale_row,
                                                                d);
    }
