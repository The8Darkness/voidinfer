...
__device__ __forceinline__ std::uint8_t unpack_oscar_conversion_code(
    ...
    // contiguous bitstream. The typed layout is only used on state-boundary
...
void cyclic_oscar_requantize_launch(
    ...
    OscarKVLayout source_layout, OscarKVLayout destination_layout) {
    ...
    if (!is_valid_oscar_kv_layout(source_layout) ||
        !is_valid_oscar_kv_layout(destination_layout)) {
        throw std::invalid_argument("OSCAR requantization layout is invalid");
    }
    const bool source_q2_transposed = source_layout == OscarKVLayout::TransposedQ2;
    const bool destination_q2_transposed = destination_layout == OscarKVLayout::TransposedQ2;
