...
template <class Geometry, int TileTokens, int ActiveTokens>
void launch_exact_wide(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    using Schedule = Q4DraftSmallTWideSchedule;
    static_assert((Geometry::kOutputRows % Schedule::kRowsPerCta) == 0);

    constexpr int kBlocks = Geometry::kOutputRows / Schedule::kRowsPerCta;
    q4_small_t_mma_kernel<Geometry, TileTokens, ActiveTokens, Q4SmallTMmaStoreEpilogue,
                          Q4SmallTMmaIdentityRows, Schedule>
        <<<kBlocks, Schedule::kThreads, 0, stream>>>(
            static_cast<const __nv_bfloat16*>(x.data),
            static_cast<const std::uint8_t*>(weight.qdata),
            static_cast<const std::uint8_t*>(weight.scales), static_cast<__nv_bfloat16*>(out.data));
...
template <class Geometry, int TileTokens, int ActiveTokens>
void launch_exact_k_wide(const Tensor& x, const Weight& weight, Tensor& out, cudaStream_t stream) {
    using Schedule = Q4DraftSmallTKWideSchedule;
    static_assert((Geometry::kOutputRows % Schedule::kRowsPerCta) == 0);
...
template <class Geometry, int First, std::size_t... Offsets>
constexpr auto make_wide_launchers(std::index_sequence<Offsets...>) {
    return std::array<Q4Launch, sizeof...(Offsets)>{
        &launch_exact_wide<Geometry, ((First + static_cast<int>(Offsets) + 7) / 8) * 8,
                           First + static_cast<int>(Offsets)>...};
}

constexpr auto kFullWideLaunchers = make_wide_launchers<FullGeometry, kFirstSmallT>(
    std::make_index_sequence<kLastFullT - kFirstSmallT + 1>{});

template <class Geometry, int First, std::size_t... Offsets>
constexpr auto make_k_wide_launchers(std::index_sequence<Offsets...>) {
    return std::array<Q4Launch, sizeof...(Offsets)>{
        &launch_exact_k_wide<Geometry, ((First + static_cast<int>(Offsets) + 7) / 8) * 8,
                              First + static_cast<int>(Offsets)>...};
}

constexpr auto kFullKWideLaunchers = make_k_wide_launchers<FullGeometry, kFirstSmallT>(
    std::make_index_sequence<kLastFullT - kFirstSmallT + 1>{});
...
void launch_q4_draft_head_small_t_wide(const Tensor& x, const Weight& weight, Tensor& out,
                                       cudaStream_t stream) {
    if (matches<FullGeometry>(x, weight) && x.ne[1] <= kLastFullT) {
        kFullWideLaunchers[static_cast<std::size_t>(x.ne[1] - kFirstSmallT)](x, weight, out,
                                                                               stream);
        return;
    }
    throw std::invalid_argument("Q4 Linear draft-head wide small-T: unsupported exact problem");
}

void launch_q4_draft_head_small_t_k_wide(const Tensor& x, const Weight& weight, Tensor& out,
                                         cudaStream_t stream) {
    if (matches<FullGeometry>(x, weight) && x.ne[1] <= kLastFullT) {
        kFullKWideLaunchers[static_cast<std::size_t>(x.ne[1] - kFirstSmallT)](x, weight, out,
                                                                                stream);
        return;
    }
    throw std::invalid_argument("Q4 Linear draft-head K-wide small-T: unsupported exact problem");
}
