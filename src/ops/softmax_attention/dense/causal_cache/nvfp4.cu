...
bool use_oscar_q2_transposed() noexcept {
    const char* value = std::getenv("NINFER_OSCAR_Q2_TRANSPOSED");
    // Full-attention OSCAR-Q2 pages are written in the lane-local transposed layout by default.
    // The older cooperative decoders below consume the contiguous compatibility layout, so never
    // route to them against the serving representation.
    return value != nullptr && value[0] == '1';
}

int oscar_xqa_split() noexcept {
    if (use_oscar_q2_transposed()) return 0;
...
void launch_oscar_attention(const Tensor& q, const Tensor& positions,
    ...
    const int split = oscar_xqa_split();
    if (split == 1) {
