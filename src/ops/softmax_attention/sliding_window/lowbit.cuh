...
__device__ __forceinline__ void sliding_window_lowbit_accumulate(
...
#pragma unroll
    for (int item = 0; item < 4; ++item) {
        ...
        const float key_value = [&]() {
            if constexpr (Packed) {
                return cyclic_kv_lowbit_decode_value<Bits>(key_codes, key_scales, d);
            }
...
#pragma unroll
    for (int item = 0; item < 4; ++item) {
        ...
        const float value = [&]() {
            if constexpr (Packed) {
                return cyclic_kv_lowbit_decode_value<Bits>(value_codes, value_scales, d);
            }
...
template <int Bits>
__launch_bounds__(32, 4) __global__ void sliding_window_attention_lowbit_kernel(
    ...
    constexpr int CodeExtent = cyclic_kv_lowbit_code_extent<Bits>(D);
    constexpr int ScaleExtent = D / kCyclicKVLowBitQuantGroup;
...
#pragma unroll
    ...
    if (protected_capacity != 0 || protected_anchor_capacity != 0) {
        ...
        for (int context_position = context_start;
            ...
            sliding_window_lowbit_accumulate<Bits, false>(
        ...
        for (int context_position = packed_begin; context_position < packed_end;
            ...
            const std::int64_t scale_offset =
                ...
                (slot + static_cast<std::int64_t>(padded_context) * kv_head);
            sliding_window_lowbit_accumulate<Bits, true>(
