...
int test_oscar_layout_contract() {
    KVPageGeometry geometry{
        .page_tokens        = kPagedKVPageSize,
        .device_plane_order = PagedKVPlaneOrder::PageMajor,
        .planes             = {},
        .oscar_layout       = OscarKVLayout::Contiguous,
    };
    const KVPageGeometry transposed = [&] {
        KVPageGeometry result = geometry;
        result.oscar_layout   = OscarKVLayout::TransposedQ2;
        return result;
    }();
    PagedKVLayerView single;
    PagedKVBatchLayerView batch;
    single.layout = OscarKVLayout::TransposedQ2;
    batch.layout  = single.layout;
    if (!is_valid_oscar_kv_layout(geometry.oscar_layout) ||
        !is_valid_oscar_kv_layout(transposed.oscar_layout) || geometry == transposed ||
        single.layout != transposed.oscar_layout || batch.layout != transposed.oscar_layout) {
        std::cerr << "OSCAR layout contract is not round-trippable\n";
        return 1;
    }
    return 0;
}
...
int main() {
    ...
    if (const int layout_failures = test_oscar_layout_contract(); layout_failures != 0) {
        return layout_failures;
    }
    if (cuda_unavailable()) {
