...
int exercise_vision(ninfer::Engine& engine, bool include_visual_mtp_bridge) {
    ...
    if (!include_visual_mtp_bridge) { return 0; }
...
int main() {
    ...
    if (qwen38_groupwise != nullptr && *qwen38_groupwise != '\0') {
        ...
        {
            ninfer::Engine vision_engine(engine_options(qwen38_groupwise));
    ...
    if (qwen38_nvfp4 != nullptr && *qwen38_nvfp4 != '\0') {
        if (const int result = exercise_host_restore(qwen38_nvfp4); result != 0) {
            return result;
        }
        {
            ninfer::Engine vision_engine(engine_options(qwen38_nvfp4));
            if (const int result = exercise_vision(vision_engine, false); result != 0) {
                return result;
            }
        }
